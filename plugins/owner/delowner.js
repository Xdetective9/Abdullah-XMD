...
module.exports = {
  name: "delowner",
  alias: ["removeowner"],
  desc: "Remove an owner number (owner only)",
  category: "owner",
  usage: "delowner <number>",
  react: "➖",
  start: async (m, ctx={}) => {
    const { args = [], isOwner=false } = ctx;
    if (!isOwner) return m.reply("❌ Owner only.");
    const num = args[0];
    if (!num) return m.reply("⚠️ Usage: delowner <number>");
    const s = load();
    s.owners = (s.owners||[]).filter(x=>x!==num);
    save(s);
    return m.reply(`✅ Removed owner: *${num}*`);
  }
};
